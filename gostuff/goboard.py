...
from gostuff.scoring import compute_game_result
...
class Board():
    ...
    def place_stone(self, player, point, sim=False):
        ...
        for other_color_string in adjacent_opposite_color:
            ...
            if replacement.num_liberties:
                ...
                self._remove_string(other_color_string, sim=sim)

    def _replace_string(self, new_string):
        for point in new_string.stones:
            self._grid[point] = new_string

    def _remove_string(self, string, sim=False):
...
class GameState():
    ...
    def apply_move(self, move, sim=False):
        if move.is_play:
            next_board = copy.deepcopy(self.board)
            next_board.place_stone(self.next_player, move.point, sim=sim)
        else:
    ...
    def does_move_violate_ko(self, player, move):
        ...
        if not self.board.will_capture(player, move.point):
            return False
        next_board = copy.deepcopy(self.board)
        next_board.place_stone(player, move.point, sim=True)
